...
type Action<T extends IActionData> = (data: T, unsafe?: object) => Promise<any>
type ActionList = { [index: number]: Action<Object> }
...
export class Flows {
  ...
  private flows: Map<string, ActionList> = new Map()
   ...
   */
  register(name: string, actions: ActionList) {
   ...
   */
  private async executeRepeat<T extends IActionData>(
    flowName: string,
    data: T,
    unsafe: object,
    i: number,
    ...
    try {
      ...
      if (typeof result !== 'object') {
        throw new Error(
          `in flow ${flowName} action number ${i} return "${result}" instead of object!\nactions must return object`
        )
      }
      ...
      this.hooks
        .get('exception')
        .forEach((fn) =>
          fn({ flowName, i, actionFn: this.flows.get(flowName)[i], input: actionData, error: error.message || error })
        )
    ...
    this.hooks
      .get('post_action')
      .forEach((fn) =>
        fn({ flowName, i, actionFn: this.flows.get(flowName)[i], input: actionData, output: nextActionData })
      )
